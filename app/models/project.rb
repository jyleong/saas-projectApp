class Project < ActiveRecord::Base
  ...
  validate :free_plan_can_only_have_one_project ## must create custom validation
  ...
  def self.by_user_plan_and_tenant(tenant_id)
    ...
    if tenant.plan == "premium"
      if user.is_admin?
        ...
        tenant.projects ##retrieve al lprojects
      else
        user.projects.where(tenant_id: tenant.id)
      end
    else
      if user.is_admin?
        tenant.projects.order(:id).limit(1) ##retrieve only 1
      else
